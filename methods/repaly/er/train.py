    ...
    for context, train_dataset in enumerate(train_datasets, 1):
        ...
        for epoch in range(max_epoch_num):
            ...
            if curr_best_accuracy_epoch > curr_best_accuracy:
                ...
                if epoch >= min_epoch_num - 1:
                    ...
                    break
        if args.save:
            test_all_so_far(model, valid_datasets, context, method='er', tofile=True, args=args)
            save_classifier(model, 'er', context, args.index)
        ...
        if checkattr(model, 'use_memory_buffer'):
            ...
            for class_id in new_classes:
                ...
                model.construct_memory_set(dataset=class_dataset, n=samples_per_class)
            model.compute_means = True

        # REPLAY: update source for replay
        if context<len(train_datasets) and hasattr(model, 'replay_mode'):
            previous_model = copy.deepcopy(model).eval()
            if model.replay_mode in ('buffer', 'all'):
                ReplayStoredData = True
                if per_context:
                    previous_datasets = []
