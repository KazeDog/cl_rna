...
from layers import fc_layer,fc_layer_split, MLP
from convs import ConvLayers,DeconvLayers
from model_utils import loss_functions as lf, modules
...
class VAE(nn.Module):
    ...
    def __init__(self, image_size, image_channels,
                 # -conv-layers
                 conv_type="standard", depth=0, start_channels=16, reducing_layers=None, conv_bn=None, conv_nl=None,
                 num_blocks=None, global_pooling=None, no_fnl=True, conv_gated=False,
                 # -fc-layers
                 fc_layers=3, fc_units=1000, fc_drop=0, fc_bn=False, fc_nl="relu", fc_gated=False, excit_buffer=True,
                 # -prior
                 prior="standard", z_dim=100, n_modes=1,
                 # -decoder
        ...
        print('self.network_output=', self.network_output)
        ...
        self.fcE = MLP(input_size=self.image_size * 5, output_size=fc_units, layers=fc_layers-1,
        ...
        self.fcD = MLP(input_size=fc_units, output_size=self.image_size * 5,
    ...
    def decode(self, z):
        ...
        image_recon = self.convD(image_features)
        # image_recon = image_recon.reshape(-1, self.image_size, 5)
        return image_recon
    ...
    def sample(self, size, sample_mode=None, **kwargs):
        ...
        X = X.reshape(-1, self.image_size, 5)
        _, X = torch.max(X.data, 2)
        return X
